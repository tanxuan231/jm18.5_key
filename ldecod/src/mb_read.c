...
static void read_inter_macroblock(Macroblock *currMB)
{
  ...
  currSlice->read_motion_info_from_NAL (currMB);	//read_motion_info_from_NAL_b_slice
  // read CBP and Coeffs  ***************************************************************
  currSlice->read_CBP_and_coeffs_from_NAL (currMB);	//read_CBP_and_coeffs_from_NAL_CAVLC_420 read_CBP_and_coeffs_from_NAL_CABAC_420
}
...
static void read_one_macroblock_p_slice_cavlc(Macroblock *currMB)
{
  ...
  else
  {
    ...
    currSlice->interpret_mb_mode(currMB);	//need fix
    ...
    {
      read_skip_macroblock(currMB);		//可以去掉
    }    
...
static void read_one_macroblock_b_slice_cavlc(Macroblock *currMB)
{
  ...
  else if (currMB->mb_type == BSKIP_DIRECT)
  {
    ...
    //--- init macroblock data ---
    init_macroblock_direct(currMB);	//初始化
