...
static void read_CBP_and_coeffs_from_NAL_CAVLC_420(Macroblock *currMB)
{
  ...
  {
    cbp = currMB->cbp;  
    read_delta_quant(&currSE, dP, currMB, partMap, SE_DELTA_QUANT_INTRA);	//read syntax
  ...
  if(cbp>15)
  {
    for (ll=0;ll<3;ll+=2)
    {
      ...
      currSlice->read_coeff_4x4_CAVLC(currMB, CHROMA_DC, 0, 0, levarr, runarr, &numcoeff);	//read syntax
    ...
    if(currMB->is_lossless == FALSE)
    {
      for (b8=0; b8 < p_Vid->num_blk8x8_uv; ++b8)
      {
        ...
        for (b4=0; b4 < 4; ++b4)
        {
          ...
          currSlice->read_coeff_4x4_CAVLC(currMB, CHROMA_AC, i + 2*uv, j + 4, levarr, runarr, &numcoeff);	//read syntax
          coef_ctr = 0;
